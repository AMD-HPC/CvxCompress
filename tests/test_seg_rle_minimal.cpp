...
static void bench_radial_sinc_sweep()
{
    ...
    printf("Benchmark: Radial sinc, scale=%.0e, k=%.1f, omega=%.1f, t=%.0f\n\n", scale, k, omega, t);
    ...
    HIPCHECK(hipCompressMaxOutputSize(plan, &comp_size));
        ...
        // --- CPU compress/decompress + timing ---
        CvxCompress compressor;
        ...
        compressor.Compress(scale, h_cpu.data(), N, N, N, bx, by, bz, cpu_compressed, cpu_len);
        std::copy(h_data.begin(), h_data.end(), h_cpu.begin());

        double cpu_fwd_total = 0, cpu_inv_total = 0;
        float cpu_cr = 0;
        for (int i = 0; i < iters; ++i) {
            std::copy(h_data.begin(), h_data.end(), h_cpu.begin());
            double wt0 = omp_get_wtime();
            cpu_cr = compressor.Compress(scale, h_cpu.data(), N, N, N, bx, by, bz, cpu_compressed, cpu_len);
            double wt1 = omp_get_wtime();
            compressor.Decompress(h_cpu.data(), N, N, N, cpu_compressed, 1, cpu_len);
            double wt2 = omp_get_wtime();
            cpu_fwd_total += (wt1 - wt0);
            cpu_inv_total += (wt2 - wt1);
        }
        float cpu_fwd_ms = (float)(cpu_fwd_total / iters * 1000.0);
        float cpu_inv_ms = (float)(cpu_inv_total / iters * 1000.0);

        // CPU error: last iteration's h_cpu has the decompressed result
    ...
    printf("Table 1: Compression ratio and throughput\n");
    printf("  %6s  %7s  %7s  %7s  %7s  %9s  %9s  %9s  %9s  %7s  %7s\n",
           "N", "MB", "CPU_CR", "GPU_CR", "CR_oh%",
           "GPU_fwd", "GPU_inv", "CPU_fwd", "CPU_inv",
           "fwd_su", "inv_su");
    printf("  %6s  %7s  %7s  %7s  %7s  %9s  %9s  %9s  %9s  %7s  %7s\n",
           "", "", "", "", "",
           "ms", "ms", "ms", "ms", "x", "x");
    for (auto& r : results) {
        printf("  %6d  %7.1f  %7.1f  %7.1f  %6.1f%%  %8.3f  %8.3f  %8.3f  %8.3f  %6.1fx  %6.1fx\n",
               r.N, r.data_MB, r.cpu_cr, r.gpu_cr, r.cr_overhead,
               r.gpu_fwd_ms, r.gpu_inv_ms, r.cpu_fwd_ms, r.cpu_inv_ms,
